...
def thread_predict(stop_event, predict_event, model, spatial_transform, threshold):
    ...
    while not stop_event.is_set():
        ...
        output_file = os.path.join(
            "real_time_test",
            f"test_output_step{inference_steps}_result{result}_{torch.sigmoid(logits).item()}.mp4",
...
if __name__ == "__main__":
    ...
    model = build_model(
        ...
        state_dict=True,
    )  # Only the state_dict of the model was saved
    ...
    predict_process = threading.Thread(
        ...
        args=(stop_event, predict_event, model, spatial_transform, threshold),
    )
